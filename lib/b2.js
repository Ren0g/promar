...
export async function createFolder(projectCode, parentPath, folderName) {
  const prefix = getFolderPrefix(
    projectCode,
    [normalizeFolderPath(parentPath), normalizeFolderPath(folderName)].filter(Boolean).join("/")
  );
...
async function countFilesUnderPrefix(prefix) {
  const keys = await listAllKeys(prefix);
  return keys.filter(
    (key) =>
      key &&
...
export async function listFolderEntries(projectCode, folderPath = "") {
  ...
  const folders = await Promise.all(
    (response.CommonPrefixes || []).map(async (item) => {
      const fullPrefix = item.Prefix || "";
      const relativePath = fullPrefix.slice(getProjectPrefix(projectCode).length).replace(/\/$/, "");
      const name = relativePath.split("/").pop() || relativePath;
      const fileCount = await countFilesUnderPrefix(fullPrefix);
      return { name, path: relativePath, fileCount, type: "folder" };
    })
  );

  const sortedFolders = folders.sort((a, b) => a.name.localeCompare(b.name, "hr"));
...
export async function listFilesRecursively(projectCode, folderPath = "") {
  const normalizedPath = normalizeFolderPath(folderPath);
  const prefix = getFolderPrefix(projectCode, normalizedPath);
  const keys = await listAllKeys(prefix);

  return keys
    .filter((key) => key && !key.endsWith("/") && key !== `${prefix}${FOLDER_MARKER}`)
    .map((key) => ({
      key,
      path: displayNameFromStoredName(key.slice(prefix.length)),
      name: displayNameFromStoredName(key.split("/").pop() || "download")
    }));
}

export async function createDownloadUrl(key, expiresIn = 60 * 30) {
  const s3 = getB2Client();
