...
async function countFilesInPrefix(prefix) {
  const keys = await listAllKeys(prefix);
  return keys.filter((key) => key && !key.endsWith("/") && !key.endsWith(`/${FOLDER_MARKER}`) && !key.endsWith(FOLDER_MARKER)).length;
}
...
export async function listFolderEntries(projectCode, folderPath = "") {
  ...
  const folders = await Promise.all(
    (response.CommonPrefixes || []).map(async (item) => {
      ...
      const fileCount = await countFilesInPrefix(fullPrefix);
      return { name, path: relativePath, type: "folder", fileCount };
    })
  );

  folders.sort((a, b) => a.name.localeCompare(b.name, "hr"));
