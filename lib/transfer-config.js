import { deletePrefix, getPortalManifestKey, readJsonObject, writeJsonObject } from "./b2";
...
function normalizeProject(projectCode, project) {
  return {
    ...
    accessPin: String(project?.accessPin || project?.editorPin || project?.crewPin || "")
  };
}
...
export async function deleteProjectCompletely(projectCode) {
  ...
  await saveTransferProjects(projects);
  await deletePrefix(`${projectCode}/`);
  return true;
}
