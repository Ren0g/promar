...
function ProjectBrowser({ session, onBackToProjects, onLogout }) {
  ...
  async function handleDownloadAllWindows() {
    setBusy(true);
    setError("");
    try {
      const response = await fetch(
        `/api/transfer/download-manifest?projectCode=${encodeURIComponent(session.projectCode)}&path=${encodeURIComponent(path)}`,
        { method: "GET" }
      );
      ...
      const safeProject = String(session.projectLabel || session.projectCode || "promar-transfer")
        .replace(/[^a-zA-Z0-9-_ ]/g, "")
        .trim()
        .replace(/\s+/g, "-") || "promar-transfer";
      ...
      triggerBrowserDownload(url, `${safeProject}-${safePath}-download.ps1`);
